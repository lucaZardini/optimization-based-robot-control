...
class DDPSolverLinearDyn(DDPSolver):
    ...
    def __init__(self, name, ddp_params, H_xx, h_x, h_s, lmbda, underact, dt, CONTROL_BOUNDS, DEBUG=False, w_bounds=0,
                 max_torque=100, beta=0.1, eps=1e-3):
    ...
    def cost_running_u(self, i, x, u):
        ...
        c_u = self.lmbda * u + self.underact * np.array(
            [0, u[1]])  # Modify accordingly the gradient of the running cost w.r.t. u
        if self.CONTROL_BOUNDS:
            barr = -self.beta * np.log(self.max_torque + self.eps + u) - self.beta * np.log(
                self.max_torque + self.eps - u)
    ...
    def cost_running_uu(self, i, x, u):
        ...
        c_uu = self.lmbda * np.eye(self.nu) + self.underact * np.diagflat(
            [0, 1])  # Modify accordingly the hessian of the running cost w.r.t. u
        if self.CONTROL_BOUNDS:
            barr = -self.beta * np.log(self.max_torque + self.eps + u) - self.beta * np.log(
                self.max_torque + self.eps - u)
            barr_u = - self.beta * barr * (1 / (self.max_torque + self.eps + u) - 1 / (self.max_torque + self.eps - u))
            barr_uu = - self.w_bounds * self.beta * (barr_u * (
                        1 / (self.max_torque + self.eps + u) - 1 / (self.max_torque + self.eps - u)) + barr * (
                                                                 -1 / (self.max_torque + self.eps + u) ** 2 - 1 / (
                                                                     self.max_torque + self.eps - u) ** 2))
            c_uu += barr_uu
        return c_uu
...
class DDPSolverDoublePendulum(DDPSolverLinearDyn):
    ...
    def __init__(self, name, robot, ddp_params, H_xx, h_x, h_s, lmbda, underact, dt, CONTROL_BOUNDS, DEBUG=False,
                 simu=None, w_bounds=0, max_torque=100, beta=0.1, eps=1e-3):
        DDPSolver.__init__(self, name, ddp_params, DEBUG)
        self.robot = robot
        self.H_xx = H_xx
        self.h_x = h_x
    ...
    def start_simu(self, x0, X_bar, U_bar, KK, dt_sim, PUSH, TORQUE_LIMITS):
        ...
        for i in range(N):
            if TORQUE_LIMITS:
                U_sim[i, :] = np.clip(U_bar[i, :] - KK[i, :, :] @ (X_sim[i, :] - X_bar[i, :]), -self.max_torque,
                                      self.max_torque)
            else:
...
if __name__ == '__main__':
    ...
    if conf.TORQUE_LIMITS:
        solver = DDPSolverDoublePendulum("doublependulum", robot, ddp_params, H_xx, h_x, h_s, lmbda, underact, dt,
                                         conf.TORQUE_LIMITS, DEBUG, simu, w_bounds, max_torque, beta, eps)
    else:
        solver = DDPSolverDoublePendulum("doublependulum", robot, ddp_params, H_xx, h_x, h_s, lmbda, underact, dt,
                                         conf.TORQUE_LIMITS, DEBUG, simu)
